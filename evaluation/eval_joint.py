...
class ImageProcessor:
    ...
    def generate_answer(self, image_url: str, image_caption: str, instruction: str) -> Optional[str]:
        try:
            ...
            for step in range(self.model_config.max_new_tokens):
                # --- Text-only ---
                llm_outputs = self.text_model(
                    input_ids=generated_ids[:, -1:] if joint_cache["text"] is not None else llm_inputs.pop("input_ids"),
                    **llm_inputs,
                    past_key_values=joint_cache["text"],
                    use_cache=True
                )
                llm_logits = llm_outputs.logits[:, -1, :]
                joint_cache["text"] = llm_outputs.past_key_values

                # --- VLM ---
                vlm_outputs = self.model(
                    input_ids=generated_ids[:, -1:] if joint_cache["vlm"] is not None else vlm_inputs.pop("input_ids"),
                    **vlm_inputs,
                    past_key_values=joint_cache["vlm"],
                    use_cache=True
                )
                vlm_logits = vlm_outputs.logits[:, -1, :]
                joint_cache["vlm"] = vlm_outputs.past_key_values

                joint_logits = self.model_config.alpha * vlm_logits + (1 - self.model_config.alpha) * llm_logits
                next_token = torch.argmax(joint_logits, dim=-1, keepdim=True)

                generated_ids = torch.cat([generated_ids, next_token], dim=-1) if generated_ids is not None else next_token
